...
import { getOrderCollectionPoint, toMapsUrl } from '@/services/collection-points.js';
...
export async function sendOrderReadyEmail(orderNumber: string, kind: 'pickup' | 'delivery'): Promise<void> {
  ...
  if (kind === 'pickup') {
    ...
    const navBtn = mapsLink
      ? `<a href="${mapsLink}" style="display:inline-block;margin-top:16px;background:#1f1b16;color:#fbf7f0;text-decoration:none;font-weight:bold;padding:12px 24px;border-radius:999px;">🧭 Navigate to collect</a>`
      : '';
    const body = `
